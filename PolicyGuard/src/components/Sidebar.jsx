export default function Sidebar({ userId, darkMode, setDarkMode, activeTab, setActiveTab, sidebarOpen, setSidebarOpen }) {
    return (
        <>
                ...
                <div className="p-6 border-t border-slate-200/50 dark:border-slate-800/50">
                    <div className="bg-slate-50/50 dark:bg-slate-800/30 rounded-2xl p-4 border border-slate-200/50 dark:border-slate-700/50 backdrop-blur-sm">
                        <div className="flex items-center gap-3 mb-4">
                            <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-sm shadow-sm">
                                👤
                            </div>
                            <div className="flex flex-col">
                                <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">User ID</span>
                                <code className="text-sm font-bold text-slate-700 dark:text-slate-200 font-mono tracking-tight">{userId.slice(0, 8)}</code>
                            </div>
                        </div>

                        <button
                            onClick={() => setDarkMode(!darkMode)}
                            className="w-full flex items-center justify-between px-4 py-2.5 rounded-xl bg-white dark:bg-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700 text-sm font-semibold text-slate-600 dark:text-slate-300 border border-slate-200/50 dark:border-slate-600/50 transition-all shadow-sm hover:shadow active:scale-[0.98]"
